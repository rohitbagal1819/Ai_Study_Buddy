...
const predefinedDecks = [
    {
        ...
        lastStudied: 'Today',
        cards: [
            { front: '2 + 2', back: '4', mastered: false },
            { front: '5 - 3', back: '2', mastered: false },
            { front: '3 * 4', back: '12', mastered: false },
            { front: '10 / 2', back: '5', mastered: false }
        ]
    },
    {
        id: 2,
        ...
        cards: [
            { front: 'What is DNA?', back: 'Deoxyribonucleic acid', mastered: false },
            { front: 'What is a cell?', back: 'Basic unit of life', mastered: false },
            { front: 'What is photosynthesis?', back: 'Process by which plants make food', mastered: false }
        ]
    },
    {
        id: 3,
        ...
        cards: [
            { front: 'When was World War I?', back: '1914-1918', mastered: false },
            { front: 'Who was the first US President?', back: 'George Washington', mastered: false },
            { front: 'What year did India gain independence?', back: '1947', mastered: false },
            { front: 'Who discovered America?', back: 'Christopher Columbus', mastered: false }
        ]
    },
    {
        id: 4,
        ...
        lastStudied: 'Today',
        cards: [
            { front: 'Hello', back: 'Hola', mastered: false },
            { front: 'Thank you', back: 'Gracias', mastered: false },
            { front: 'Yes', back: 'Sí', mastered: false },
            { front: 'No', back: 'No', mastered: false }
        ]
    }
];
...
function rateCard(rating) {
    const card = currentDeck.cards[currentCardIndex];
    if (rating === 'easy' || rating === 'medium' || rating === 'done') {
        if (!card.mastered) {
            card.mastered = true;
            currentDeck.mastered++;
            currentDeck.due = Math.max(0, currentDeck.due - 1);
        }
    ...
    currentDeck.progress = Math.round((currentDeck.mastered / currentDeck.total) * 100);
    currentDeck.lastStudied = 'Today';
...
function setupEventListeners() {
    // Flip card and show ratings
    const flashcard = document.getElementById('flashcard');
    flashcard.addEventListener('click', () => {
        flashcard.classList.toggle('flipped');
        const isFlipped = flashcard.classList.contains('flipped');
        document.getElementById('ratingControls').style.display = isFlipped ? 'flex' : 'none';
    });
    ...
    document.getElementById('deckForm').addEventListener('submit', (e) => {
        ...
        cardItems.forEach(item => {
            ...
            if (front && back) {
                cards.push({ front, back, mastered: false });
            }
        });
